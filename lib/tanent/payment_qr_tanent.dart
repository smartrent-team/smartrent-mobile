...
import 'package:smartrent_mobile/tanent/payment_success_tanent.dart';

class PaymentQRTanent extends StatefulWidget {
  const PaymentQRTanent({super.key});

  @override
  State<PaymentQRTanent> createState() => _PaymentQRTanentState();
}

class _PaymentQRTanentState extends State<PaymentQRTanent>
  ...
  Widget _buildQrCard() {
    return Container(
      ...
      child: Column(
        children: [
          // QR Code (fake pixel art QR look)
          Stack(
            alignment: Alignment.center,
            children: [
              _buildFakeQR(),
  ...
  Widget _buildFakeQR() {
    ...
    // Force finder pattern corners (top-left, top-right, bottom-left)
        ...
        child: ElevatedButton.icon(
          onPressed: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (_) => const PaymentSuccessTanent(),
              ),
            );
          },
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int r = 0; r < grid.length; r++) {
      for (int c = 0; c < grid[r].length; c++) {
        ...
        // Skip cells that overlap with logo area
